...
#showCar #sub {
	background-image: url(/shopping/images/car/buy.jpg);
	width: 140px;
	height: 28px;
	border: 0px;
}
		...
		<script type="text/ecmascript" src="/shopping/js/area.js">
</script>
		<script charset="utf-8" type="text/ecmascript"
			src="/shopping/js/lookup.js">
</script>
...
$(document).ready(function() {
	...
	<body>
		<c:if
			test="${sessionScope.forder.ftotal==null || sessionScope.forder.ftotal==''}">
			<%
				request.getRequestDispatcher("/users/showCar.jsp").forward(
							request, response);
			%>
		</c:if>
			...
			<div id="mid" style="margin-bottom: 20px;">
				...
				<div>
					<div style="margin-left: 8px;">
						<b style="color: #333333">订购人信息</b>
					</div>
					<div
						style="margin: 20px 0px; margin-left: 8px; border: 1px solid #cccccc"
						id="digouren">
						...
						<div class="dizhi" style="padding: 3px 0px; padding-left: 45px;">
							<span style="margin-right: 10px;">通讯地址：</span><span>${sessionScope.users.uaddress
								}</span>
						</div>
						<div class="dizhi" style="padding: 3px 0px; padding-left: 45px;">
							<span style="margin-right: 10px;">通讯电话：</span><span>${sessionScope.users.uphone
								}</span>
						</div>
						<div class="dizhi" style="padding: 3px 0px; padding-left: 45px;">
							<span style="margin-right: 10px;">电子邮件：</span><span>${sessionScope.users.uemail
								}</span>
						</div>
					</div>
				</div>
				<div>
					<div style="margin-left: 8px;">
						<b style="color: #333333">确认收货地址</b>
					</div>
					<div
						style="margin-top: 20px; margin-left: 8px; padding: 10px 0px; border: 1px solid #cccccc"
						id="xuanzhi">
						<div class="dizhi" id="diz" style="padding-left: 45px;">
							...
							<c:choose>
								<c:when test="${param.uname=='' || param.uname== null }">
									...
									<span>${sessionScope.users.uname }</span>
									<label>
										电话:
									</label>
									<span> ${sessionScope.users.uphone }</span>
									<label>
										送货地址:
									</label>
									<span>${sessionScope.users.uaddress}</span>
									<label>
										邮政编码:
									</label>
									<span>${sessionScope.users.upost }</span>
								</c:when>
								<c:otherwise>
									<label>
										收货人：
									</label>
									<span>${param.uname }</span>
									<label>
										电话:
									</label>
									<span>${param.uphone }</span>
									<label>
										送货地址:
									</label>
									<span>${param.selectp}-${param.selectc}-${param.gxremark}</span>
									<label>
										邮政编码:
									</label>
									<span>${param.upost }</span>
								</c:otherwise>
							</c:choose>
						</div>
				...
				<div>
					...
					<div id="showCar">
						...
						<div class="ftotal">
							商品总价:￥${sessionScope.forder.ftotal}
						</div>
						<form action="/shopping/forderAction_savefoder.action"
							method="post">
							<div style="margin-top: 10px;">
								<b
									style="color: #333333; font-size: 12px; float: left; margin-right: 5px;">给卖家留言:</b>
								<textarea name="fremark" cols="60" rows="2"></textarea>
							</div>
							<div class="ftotal">
								<c:choose>
									<c:when test="${param.uname=='' || param.uname==null }">
										<input type="hidden" name="forder.uaddress"
											value="${sessionScope.users.uaddress }" />
										<input type="hidden" name="forder.uname"
											value="${sessionScope.users.uname }" />
										<input type="hidden" name="forder.uphone"
											value="${sessionScope.users.uphone }" />
										<input type="hidden" name="forder.uemail"
											value="${sessionScope.users.uemail }" />
										<input type="hidden" name="forder.upost"
											value="${sessionScope.users.upost}" />
									</c:when>
									<c:otherwise>
										<input type="hidden" name="forder.uaddress"
											value="${param.selectp}-${param.selectc}-${param.gxremark}" />
										<input type="hidden" name="forder.uname"
											value="${param.uname }" />
										<input type="hidden" name="forder.uphone"
											value="${param.uphone }" />
